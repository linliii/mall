...
public class RedisUtils {
  ...
  public <K> boolean expireAt(K k, LocalDateTime date) {
    return redisTemplate.expireAt(k, Date.from(date.atZone(ZoneId.systemDefault()).toInstant()));
  }
  ...
  public <K, V> void setExpireAt(K k, V v, LocalDateTime date) {
    ...
    redisTemplate.expireAt(k, Date.from(date.atZone(ZoneId.systemDefault()).toInstant()));
  }
  /** 同时设置多个缓存 */
